...
search = "BFS"  # Алгоритм пошуку шляху: "G" (жадібний) або "A*"
...
def graph():
    ...
    for row in Cells:
        for c in row:
            ...
            if c.Walls[0] == 1:
                pygame.draw.line(screen, (0, 0, 255), (left + c.posx + sizekoef, top + c.posy),
                                 (left + c.posx + sizekoef, top + c.posy + sizekoef))
            # Якщо є стіна вгору, малюємо її
            if c.Walls[1] == 1:
                pygame.draw.line(screen, (0, 0, 255), (left + c.posx, top + c.posy),
            ...
            if c.Walls[3] == 1:
                pygame.draw.line(screen, (0, 0, 255), (left + c.posx, top + c.posy + sizekoef),
                                 (left + c.posx + sizekoef, top + c.posy + sizekoef))
...
current_level = 1
ghost_increment = 3  # Increase ghost number by 2 each level
difficulty_increment = 25  # Increase difficulty by 25 each level
